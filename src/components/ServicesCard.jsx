    ...
    return (
        <div className={`w-full mb-[60px] lg:mb-20 flex flex-col md:flex-row md:gap-5 justify-center items-center md:justify-normal text-black ${itemsOrder === 'acc' ? '' : 'md:flex-row-reverse' }`}>
            ...
            <div className="px-[30px] md:px-0 pb-5">
                <img className="min-w-[275px] lg:min-w-[346px] xl:min-w-[414px]" width={'275px'} src={`/src/assets/${image}`} alt="service image" />
            </div>

            {/* Title */}
            <div className="flex flex-col items-center md:items-start md:py-[18px] lg:py-[79.5px] xl:py-[113.5px]">
                <h3 className="pb-5 font-semibold text-primary text-center md:text-left text-[27px] [line-height: 33px] [letter-spacing: 0.4000000059604645px]">{title}</h3>

                {/* Description */}
                <p className="text-base text-s font-normal pb-5 [line-height: 19.36px] text-center md:text-left">{description}</p>
